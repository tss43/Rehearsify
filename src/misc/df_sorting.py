...
def sort_df(score_df: pd.DataFrame, ignore_str: str) -> pd.DataFrame:
    """ Sort the score df alphabetically by answer, ignoring the supplied characters. """   

    def reduce_series(ans: pd.Series) -> pd.Series:
        """ Reduce a Series by ignoring the supplied characters and special characters/punctuation/case """
        
        ans = ans.str.replace( pat=fr'{ignore_str}', repl='', regex=True )
        ans = ans.apply( unidecode.unidecode )
        ans = ans.str.translate( str.maketrans('', '', string.punctuation) )
        ans = ans.str.lower()
