...
def compute_statistics(score_df: pd.DataFrame) -> dict:
    ...
    perc_unseen_questions = 100 * n_unseen_questions / n_translations if n_translations>0 else np.NaN
    n_practised_questions = score_df['total'].sum()
    n_wrongly_answered_questions = score_df['wrong'].sum()
    perc_wrongly_answered_questions = (
        100 * n_wrongly_answered_questions / n_practised_questions) if n_practised_questions>0 else np.NaN
