...
class RehearsifyGUI:
    ...
    def __init__(self, window):
        ...
        self.user_answer = tk.StringVar(
            self.window, value='(user input answers)')
    ...
    def initialise_GUI(self):
        ...
        def define_widgets(self):
            ...
            self.btn_update = tk.Button(
                self.button_frame, text="Update with...", command=self.update_file)
            self.btn_save = tk.Button(
                self.button_frame, text="Save as...", command=self.save_file)
            self.btn_lookup_question = tk.Button(
                self.button_frame, text="Lookup question", command=self.lookup_question)
            self.btn_lookup_answer = tk.Button(
                self.button_frame, text="Lookup answer", command=self.lookup_answer)
            self.btn_dictionary_stats = tk.Button(
                self.button_frame, text="Dict statistics", command=self.display_dictionary_stats)

            self.question_answer_frame = tk.Frame(
                self.window, relief='raised', bd=2)
            self.question_prompt = tk.Label(
                self.question_answer_frame, textvariable=self.question)
            self.equal_sign = tk.Label(self.question_answer_frame, text=" = ")
            self.answer_entry = tk.Entry(
                self.question_answer_frame, textvariable=self.user_answer)
            self.answer_entry.bind(
                '<Return>', lambda event: self.process_answer())
            self.go_btn = tk.Button(
                self.question_answer_frame, text="Go", command=self.process_answer)
            ...
            self.counter = tk.Label(self.lower_frame, font=(
                '', 10), text="session wrong/total practised: -/-")
            self.stats = tk.Label(self.lower_frame, font=(
                '', 10), text="overall wrong/total practised: -/-")
        ...
        def place_widgets_on_grid(self):
            ...
            self.question_prompt.grid(
                row=0, column=0, sticky='NSEW', padx=0, pady=15)
            self.equal_sign.grid(
                row=0, column=1, sticky='NSEW', padx=0, pady=15)
            self.answer_entry.grid(
                row=0, column=2, sticky='NSEW', padx=0, pady=15)
            ...
            self.mark_correct_btn.grid(
                row=0, column=0, rowspan=2, sticky='W', padx=5)
    ...
    def initialise_log(self):
        ...
        self.log.column('#1', width=35, minwidth=35,
        ...
        self.log.column('#5', width=75, minwidth=75,
                        stretch=tk.NO, anchor=tk.CENTER)
    ...
    def open_file(self):
        ...
        self.stats.config(
            ...
            + str(self.prev_practise_wrong_count +
                  self.practise_wrong_count) + '/'
    ...
    def process_answer(self):
        ...
        self.score_df[self.score_df['question']
                      == self.sample.question] = self.sample

        # update log treeview widget
        update_dict = self.dict_to_insert_in_log(
            self.sample, self.user_answer, answer_is_correct)
        self.log.insert('', index=0, iid=self.practise_count,
    ...
    def lookup_answer(self):
        ...
        if (answer := askstring(title="Answer lookup", prompt="Answer for which to find the sample:")):
            try:
                ...
                # update log treeview widget
                update_dict = update_dict = self.dict_to_insert_in_log(
                    _sample, "", None)
                self.log.insert('', index=0, values=list(update_dict.values()))
    ...
    def mark_correct(self):
        ...
        previous_question = self.log.set(
            item=str(self.practise_count), column='Question')
        previous_correct_answer = self.log.set(
            item=str(self.practise_count), column='Correct answer')
        previous_user_answer = self.log.set(
            item=str(self.practise_count), column='User answer')

        previous_user_answer_is_correct = check_answer(
            previous_user_answer, previous_correct_answer)
        ...
        if previous_user_answer and not previous_user_answer_is_correct:
            ...
            _sample = self.score_df[self.score_df['question']
                                    == previous_question].sample(n=1).squeeze()
            ...
            # update log treeview widget
            update_dict = self.dict_to_insert_in_log(
                _sample, previous_user_answer, True)
            self.log.delete(str(self.practise_count))
            self.log.insert('', index=0, iid=str(
                self.practise_count), values=list(update_dict.values()))
...
class SortingPopUp:
    ...
    def __init__(self, master):
        ...
        # defining widgets
        self.lbl = tk.Label(
            self.top, text="""Preferred sorting order:""", justify='left')
        ...
        self.top.bind('<Return>', lambda event: self.btn_parsing(
            v=btn_var, e1=self.entry_answer, e2=self.entry_question))

        # set default sorting function if cancelled (through button or manually)
        self.sort_df_func = no_sort_df
        self.btn_cancel = tk.Button(
            self.btn_frame, text="Cancel", command=self.top.destroy)
        ...
        self.entry_question.grid(
            row=8, column=1, sticky='NSEW', padx=0, pady=1)
    ...
    def btn_parsing(self, v, e1=None, e2=None):
        ...
        match v.get():
            ...
            case 3:
                ...
                self.sort_df_func = partial(
                    sortby_str_df, sortby='answer', ignore_str=ignore_str)
            case 4:
                ignore_str = self.get_valid_str_from_entry(e2)
                self.sort_df_func = partial(
                    sortby_str_df, sortby='question', ignore_str=ignore_str)
    ...
    def get_valid_str_from_entry(self, entry) -> str:
        ...
        while entry_str is None:
            try:
                ...
                # restetting entry widget
                entry_str = None
                entry.delete(0, 'end')

                # demanding new entry value from user
