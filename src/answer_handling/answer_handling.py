...
def update_score_df( score_df, question, answer_is_correct ) -> pd.DataFrame:
    """ update the score dataframe after a receiving a right/wrong answer on the question from the user"""
    
    score_df.loc[ score_df['question']==question, 'total' ] += 1
    if not answer_is_correct: # in case the user answer was wrong, also add 1 to the 'wrong' column
        score_df.loc[ score_df['question']==question, 'wrong' ] += 1
